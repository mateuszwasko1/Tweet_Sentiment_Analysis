...
class Preprocessing_Test(unittest.TestCase):
    ...
    def test_extract_feature_lables(self):
        d = {'tweet': ["This is a angry tweet", "This is a sad tweet"],
             "emotion": ["angry", "sadness"]}
        df = pd.DataFrame(data=d)

        X, y = self.processor.extract_features_labels(df, "tweet", "emotion")

        actual_X = pd.Series(["This is a angry tweet", "This is a sad tweet"],
                             name="tweet")
    ...
    def test_vectorize(self):
        ...
        X_train = pd.Series(["This is a angry tweet", "This is a sad tweet"],
                            name="tweet")
        X_train_vec = self.processor.vectorize(X_train)
        # The shape should be 2,5 because we have 2 data points and the
        # longest one is 5 words long
        self.assertEqual(X_train_vec.shape, (2, 5))

        # Testing for test/dev set
        self.processor.training_data = False
        X_test = pd.Series(["This is a angry tweet in test data set"],
                           name="tweet")
